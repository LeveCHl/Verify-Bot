...
- 同じユーザーが再度認証ボタンを押しても、ロールは重複付与されず「既に認証済みです」と表示されます
- 同じIPアドレスから**別のDiscordアカウント**が過去に認証している場合、管理者ログに⚠️警告として
  該当アカウント一覧が表示されます(複垢・荒らし対策)。これはあくまで参考情報であり、
  自動BANなどは行いません。最終判断は管理者が行ってください
